...
pacman::p_load(cutpointr, ggplot2, plyr, dplyr, ggrepel,knitr,ggbiplot)
...
table1(~Sex+Age+Race+Diagnosis+Follow.Up+PatientwithAct+TTanyactivity+TTMRIActivity+TTAttack+
...
variable<- "Total.Diagnosis.Lesion"
...
reg_val<- as.data.frame(reg_val)
reg_val<- as.data.frame(t(reg_val))
p_values<- na.omit(reg_val)
qobj <- qvalue(p=p_values$V1, pi0 = 1)
...
data2<- data_combo[MSNA_only_vd,] #Select subset of data
...
model_test<- lm(Activity~IgG1_CXCL10+TX+Age+Sex+BMI+CCL11_CCL13 +IgM_CCL22 +IFNG_CCL13 +CXCL10_CXCL9,
                data = use_me2)
summary(model_test)
sink(paste(holder, ".txt")) ####change####
